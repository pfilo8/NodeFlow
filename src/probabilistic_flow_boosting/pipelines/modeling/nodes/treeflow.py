...
def train_treeflow(x_train, y_train, flow_p, flow_params, tree_p, tree_params, tree_model_type, n_epochs: int = 100,
...
def modeling_treeflow(x_train: pd.DataFrame, y_train: pd.DataFrame, tree_model_type, flow_params, tree_params,
    ...
    for flow_p in generate_params_for_grid_search(flow_hyperparams):
        for tree_p in generate_params_for_grid_search(tree_hyperparams):
            m = train_treeflow(x_tr, y_tr, flow_p, flow_params, tree_p, tree_params, tree_model_type,
    ...
    m = train_treeflow(x_train, y_train, best_flow_p, flow_params, best_tree_p, tree_params, tree_model_type,
                       n_epochs, batch_size, random_seed)
    return m
