...
class SoftDecisionTree(nn.Module):
    ...
    def forward_leaves(self, X):
        """Forward pass through model without final layer of prediction, i.e., probability on leaves."""
        _mu, _penalty = self._forward(X)
        return _mu, _penalty
    ...
    def _validate_parameters(self):
        if not self.depth > 0:
            raise ValueError(
                f"The tree depth should be strictly positive, but got {self.depth}"
            )

        if not self.lamda >= 0:
            raise ValueError(
                f"The coefficient of the regularization term should not be negative, but got {self.lamda} instead."
            )
